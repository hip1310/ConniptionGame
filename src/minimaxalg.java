...
//	    ArrayList<node> sameutil = new ArrayList<node>();
	   ...
	   // node returned = new node(sameutil.get(0).getBoardstate(),sameutil.get(0).getActionmenu(),null,"max",0);
	    node returned = new node(rootnode.getChildArray().get(move).getBoardstate(),rootnode.getChildArray().get(move).getActionmenu(),null,"max",0);
	    //System.out.println("Computer's move : " + sameutil.get(0).getActionmenu());
	    System.out.println("Computer's move : " + rootnode.getChildArray().get(move).getActionmenu());
	    return returned;
	...
	private int minmax(node n,int depth, int min, int max){
		...
		if(n.gethasChild()==false && depth==0){
			...
			//return n.hybrid1(n.getBoardstate(), 'O');
			int util = n.hybrid3(n.getBoardstate(), 'O', n.getFlipPenalty());
			n.setEvalFunction(util);
			return util;
		}
		//node is max
