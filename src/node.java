...
public class node {
	...
	private static int[][] evaluationTable ={{3, 4, 5, 7, 5, 4, 3}, 
											...
											{3, 4, 5, 7, 5, 4, 3}};
/*
	private static int[][] evaluationTable ={{1, 2, 3, 5, 3, 2, 1}, 
											{2, 4, 6, 7, 6, 4, 2},
											{3, 6, 8, 9, 8, 6, 3}, 	
											{3, 6, 8, 9, 8, 6, 3},
											{2, 4, 6, 7, 6, 4, 2},
...
*/	
	...
	*/	
		...
		//System.out.println(priority1 + " " + priority2 + " " + priority3 + " " + priority4 + " " + flipPenalty);
		//System.out.println((5 * priority1) + " " + (priority2) + " " + (2 * priority3) + " " + (4  * priority4) + " " + (4 * flipPenalty));
    
		return (4 * priority1) + (2 * priority2) + (3 * priority4) - (2 * flipPenalty);
	
	}
	//using evalution table 
	public int eval1(char[][] board){
   ...
   public int topstack(){
       ...
       for (int col = 0; col < columns; col++) {
           ...
           if(temp.size()>0 && temp.peek() == curChecker)
               numOattop++;
           else if(temp.size()>0 && temp.peek() == oppChecker)
               numXattop++;
       }
       
       return ((5 * numOattop) - (2 *numXattop));
   }
	
	public int isGoal(char[][] board) {
		...
		for (int row=0; row<rows; row++){ 
		    for (int col=0; col<columns-3; col++){
		    	if (board[row][col] != ' ' &&
		    		...
		    		if(board[row][col] == curChecker) countWinAI++;
					if(board[row][col] == oppChecker) countWinHu++;
		    }
		}   
		...
		return ((6 * countWinAI)- (3 * countWinHu));
	}
	...
	public int is3inRow(char[][] board) {
		...
		return ((3 *((count3AI )+(2 * count2AI))) - ((2 * count3Hu) + (count2Hu)));
	}
